...
public:
	...
	void instert(unsigned int Index, unsigned int count, T data);    //在Index处添加count个数据为data的元素
...
template <class T>
void linkList<T>::instert(unsigned int Index, unsigned int count, T data)
{
	ListNode<T> *temp, *temp2;
	size_t i = 0;
	if (Index < 0 || count <= 0)
